# AGENTS.md — kyoka-grid

強化学習グリッドワールド可視化。Q学習の学習過程(価値・方策・軌跡)をリアルタイム描画し、
価値反復オラクルとの一致率で収束を検証する教材アプリ。仕様は SPEC.md、テストは TEST_SPEC.md。
...
学習ゲート G-xx の基準変更、`.wt/gate.json` の上限変更)は、人間の承認なしに行わない。
...
- 乱数はシード付き PRNG を**注入**する(F-01)。core 内で `Date.now()` / `Math.random()` を直接呼ばない。
  乱数状態は学習状態(TrainerState)の一部として持ち回り、同一入力 → 同一出力を保証する。
